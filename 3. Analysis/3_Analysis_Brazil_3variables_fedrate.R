...
#brazil_epu <- window(fedrate.1997, start = c(1998,1))
#brazil_fdi <- window(brazil_fdi, start = c(1998,1))
#brazil_pi <- window(brazil_pi, start = c(1998,1))

# FDI: First ordering ####
Y <- cbind(fedrate.1997, brazil_epu, brazil_fdi)
...
# Check: IRFs
#irf1.1 <- irf(SVAR , impulse = "fedrate.1997", response = "brazil_epu", n.ahead = 8, ortho = TRUE, runs=1000, seed=101)
...
# FDI: Second ordering ####
Y <- cbind(fedrate.1997, brazil_fdi, brazil_epu)
Y.d <- diff(Y) # Data in differences
#plot(Y.d)
...
# PI: First ordering ####
Y <- cbind(fedrate.1997, brazil_epu, brazil_pi)
Y.d <- diff(Y) # Data in differences
#plot(Y.d)
...
# PI: Second ordering ####
Y <- cbind(fedrate.1997, brazil_pi, brazil_epu)
Y.d <- diff(Y) # Data in differences
#plot(Y.d)
