...
# FDI: First ordering ####
Y <- cbind(chile_epu, chile_gdp, chile_fdi)
...
# Check: IRFs
#irf1.1 <- irf(SVAR , impulse = "chile_gdp", response = "chile_epu", n.ahead = 8, ortho = TRUE, runs=1000, seed=101)
...
# FDI: Second ordering ####
Y <- cbind(chile_gdp, chile_fdi, chile_epu)
Y.d <- diff(Y) # Data in differences
#plot(Y.d)
...
# PI: First ordering ####
Y <- cbind(chile_gdp, chile_pi, chile_epu)
Y.d <- diff(Y) # Data in differences
#plot(Y.d)
