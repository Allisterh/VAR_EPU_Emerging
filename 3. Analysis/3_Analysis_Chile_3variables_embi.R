...
# Check: IRFs
#irf1.1 <- irf(SVAR , impulse = "embi", response = "chile_epu", n.ahead = 8, ortho = TRUE, runs=1000, seed=101)
...
# FDI: Second ordering ####
Y <- cbind(embi, chile_fdi, chile_epu)
Y.d <- diff(Y) # Data in differences
#plot(Y.d)
...
# PI: First ordering ####
Y <- cbind(embi, chile_epu, chile_pi)
Y.d <- diff(Y) # Data in differences
#plot(Y.d)
...
# PI: Second ordering ####
Y <- cbind(embi, chile_pi, chile_epu)
Y.d <- diff(Y) # Data in differences
#plot(Y.d)
