...
#chile_epu <- window(global_epu, start = c(1998,1))
#chile_fdi <- window(chile_fdi, start = c(1998,1))
#chile_pi <- window(chile_pi, start = c(1998,1))

# FDI: First ordering ####
Y <- cbind(global_epu, chile_epu, chile_fdi)
...
# Check: IRFs
#irf1.1 <- irf(SVAR , impulse = "global_epu", response = "chile_epu", n.ahead = 8, ortho = TRUE, runs=1000, seed=101)
...
# FDI: Second ordering ####
Y <- cbind(global_epu, chile_fdi, chile_epu)
Y.d <- diff(Y) # Data in differences
#plot(Y.d)
...
# PI: First ordering ####
Y <- cbind(global_epu, chile_epu, chile_pi)
Y.d <- diff(Y) # Data in differences
#plot(Y.d)
...
# PI: Second ordering ####
Y <- cbind(global_epu, chile_pi, chile_epu)
Y.d <- diff(Y) # Data in differences
#plot(Y.d)
