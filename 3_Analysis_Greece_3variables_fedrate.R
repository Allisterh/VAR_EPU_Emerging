...
#greece_epu <- window(fedrate.1997, start = c(1998,1))
#greece_fdi <- window(greece_fdi, start = c(1998,1))
#greece_pi <- window(greece_pi, start = c(1998,1))

# FDI: First ordering ####
Y <- cbind(fedrate.1997, greece_epu, greece_fdi)
...
# Check: IRFs
#irf1.1 <- irf(SVAR , impulse = "fedrate.1997", response = "greece_epu", n.ahead = 8, ortho = TRUE, runs=1000, seed=101)
...
# FDI: Second ordering ####
Y <- cbind(fedrate.1997, greece_fdi, greece_epu)
Y.d <- diff(Y) # Data in differences
#plot(Y.d)
...
# PI: First ordering ####
Y <- cbind(fedrate.1997, greece_epu, greece_pi)
Y.d <- diff(Y) # Data in differences
#plot(Y.d)
...
# PI: Second ordering ####
Y <- cbind(fedrate.1997, greece_pi, greece_epu)
Y.d <- diff(Y) # Data in differences
#plot(Y.d)
