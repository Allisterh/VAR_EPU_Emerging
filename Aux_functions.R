...
#graficos <- function(SVAR_name, width, height, H){
#    
#  for(i in 1:dim(Y)[1]) {
#  for(j in 1:dim(Y)[2]) {
#        
...
#  graficos <- function(SVAR_name, width, height, H){
 ...
 # png(file=paste(country, deparse(substitute(SVAR_name)), ".png", sep=""), width= width, height= height)
 # plot1 <- plot.sirf.boot(eval(as.name(paste(deparse(substitute(SVAR_name)), ".SIRF.c.boot", sep=""))), m, H, pos_g)
 # print(plot1)
 # dev.off()
 # 
