...
# FDI: First ordering ####
Y <- cbind(global_epu, greece_epu, greece_fdi)
...
# Check: IRFs
#irf1.1 <- irf(SVAR , impulse = "global_epu", response = "greece_epu", n.ahead = 8, ortho = TRUE, runs=1000, seed=101)
...
# FDI: Second ordering ####
Y <- cbind(global_epu, greece_fdi, greece_epu)
Y.d <- diff(Y) # Data in differences
#plot(Y.d)
...
# PI: First ordering ####
Y <- cbind(global_epu, greece_epu, greece_pi)
Y.d <- diff(Y) # Data in differences
#plot(Y.d)
...
# PI: Second ordering ####
Y <- cbind(global_epu, greece_pi, greece_epu)
Y.d <- diff(Y) # Data in differences
#plot(Y.d)
